...
const NavButton = ({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) => (
  <button onClick={onClick} className={`group w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 ${active ? 'bg-orange-50 text-orange-600 font-semibold shadow-sm' : 'text-stone-500 hover:bg-stone-50 hover:text-stone-900'}`}>
...
const MobileNavButton = ({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center w-full h-full rounded-xl transition-colors ${active ? 'text-orange-600' : 'text-stone-400'}`}>
    <div className={`p-1.5 rounded-full mb-0.5 transition-all ${active ? 'bg-orange-50 -translate-y-1' : ''}`}>{icon}</div>
